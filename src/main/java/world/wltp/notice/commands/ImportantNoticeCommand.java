...
public class ImportantNoticeCommand implements CommandExecutor {

    private final JavaPlugin plugin;

    public ImportantNoticeCommand(JavaPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("notice.use")) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                URL url = new URL("https://www.wltp.world/api/important_notices/");
                ...
                if (args.length == 0) {
                    // 📋 리스트 출력 모드
                    player.sendMessage(ChatColor.YELLOW + "[중요공지 목록]");
                    ...
                    try {
                        ...
                        if (index >= 0 && index < notices.length()) {
                            ...
                            player.sendMessage(ChatColor.YELLOW + "[중요공지 - " + (index + 1) + "번]");
                        ...
                        player.sendMessage(ChatColor.RED + "숫자를 입력해주세요. 예: /중요공지 2");
                    }
                }
