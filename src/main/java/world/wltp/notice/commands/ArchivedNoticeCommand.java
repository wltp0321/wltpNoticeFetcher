...
public class ArchivedNoticeCommand implements CommandExecutor {

    private final JavaPlugin plugin;

    public ArchivedNoticeCommand(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                URL url = new URL("https://www.wltp.world/api/archived_notices/");
                ...
                if (args.length == 0) {
                    // 📋 리스트 출력 모드
                    player.sendMessage(ChatColor.YELLOW + "[아카이브공지 목록]");
                    ...
                    try {
                        ...
                        if (index >= 0 && index < notices.length()) {
                            ...
                            player.sendMessage(ChatColor.YELLOW + "[아카이브공지 - " + (index + 1) + "번]");
                        ...
                        player.sendMessage(ChatColor.RED + "숫자를 입력해주세요. 예: /아카이브공지 2");
                    }
                }
